...
export class GetNearProgressSummaryOfStudent {
  constructor(private readonly learningRepo: LearningRepository) {}

  async execute(studentId: number, courseId: number): Promise<StudentNearProgressData[]> {
    ...
    const [
      recentTeacherReviews,   // expect fields: get_day (YYYY-MM-DD), teacher_reviews (string | string[])
    ...
    const getDay = (row: AnyObj): string | undefined =>
      row?.get_day ?? row?.submitted_day ?? row?.date;
    ...
    const reviewsMap = recentTeacherReviews.reduce<Record<string, string>>((acc, r: AnyObj) => {
      const day = getDay(r);
      if (!day) return acc;
    ...
    // 2) Badges per day
    const badgesMap = recentBadges.reduce<Record<string, number>>((acc, b: AnyObj) => {
      const day = getDay(b);
      if (!day) return acc;
    ...
    const vsMap = recentVideoSessions.reduce<Record<string, number>>((acc, v: AnyObj) => {
      const day = getDay(v);
      if (!day) return acc;
    ...
    // 4) Homework per day
    const hwMap = recentHomework.reduce<Record<string, number>>((acc, h: AnyObj) => {
      const day = getDay(h);
      if (!day) return acc;
    ...
    const rankMap = recentEffortRank.reduce<Record<string, number>>((acc, e: AnyObj) => {
      const day = getDay(e);
      if (!day) return acc;
      const r = Number(e.ranking ?? e.rank);
      if (Number.isNaN(r)) return acc;
      acc[day] = acc[day] == null ? r : Math.min(acc[day], r);
      return acc;
    }, {});

    // 6) Video quiz correct count
    const quizMap = recentVideoQuiz.reduce<Record<string, number>>((acc, q: AnyObj) => {
      const day = getDay(q);
      if (!day) return acc;
    ...
    const result: StudentNearProgressData[] = last7Days.map((day) => ({
      label: day,                              // date
