...
import { InteractionTrigger } from '../../../domain/IM/models/interaction-trigger.model';
import { pool } from '../../common/db-postgres'; // dùng DB chung

export class PgEventRepository implements EventRepository {
  async create(e: InteractionTrigger): Promise<number> {
  ...
  async findById(id: number): Promise<InteractionTrigger | null> {
    const r = await pool.query('SELECT * FROM im_events WHERE id=$1', [id]);
