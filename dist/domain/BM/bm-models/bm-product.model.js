...
exports.parseBMProductRow = parseBMProductRow;
exports.parseBMProductRows = parseBMProductRows;
/**
 * Parse 1 row từ DB (snake_case) sang domain model BMProduct (camelCase).
 */
function parseBMProductRow(input) {
    assertObject(input, 'bm_product row must be an object');
    const { id, product_code, product_type, product_title, tutorial_video_url, sale_kit_url, user_guide_link, success_stories, category, subject, grade, level, name, tagline, description, thumbnail_url, price_amount, list_price_amount, price_currency, promotion_flag, promotion_note, sale_start_at, sale_end_at, access_start_at, access_end_at, access_duration_days, status, is_visible, display_order, target_student_desc, learning_outcome, metadata, created_at, created_by, updated_at, updated_by, } = input;
    ...
    const listPriceAmountNum = toOptionalNumberLike(list_price_amount, 'list_price_amount must be number or null');
    if (!product_code) {
    ...
    const accessDurationDaysNum = toOptionalIntegerLike(access_duration_days, 'access_duration_days must be integer or null');
    const displayOrderNum = toOptionalIntegerLike(display_order, 'display_order must be integer or null');
    // ===== Metadata (JSONB) =====
    let parsedMetadata = null;
    if (metadata !== null && metadata !== undefined) {
        if (typeof metadata === 'string') {
            parsedMetadata = safeJsonParse(metadata, null);
        }
        else {
    ...
    let parsedSuccessStories = null;
    if (success_stories !== null && success_stories !== undefined) {
        let raw = success_stories;
        if (typeof success_stories === 'string') {
            raw = safeJsonParse(success_stories, null);
        }
        if (Array.isArray(raw)) {
            ...
            raw.forEach((item, idx) => {
                ...
                const imageUrlRaw = item.image_url !== undefined ? item.image_url : item.imageUrl;
                const titleRaw = item.title;
                const storyRaw = item.story;
                if (typeof titleRaw !== 'string' || typeof storyRaw !== 'string') {
                    throw new Error(`success_stories[${idx}] must have title and story as string`);
                }
 ...
 */
function parseBMProductRows(input) {
...
function toOptionalIntegerLike(val, msg) {
    if (val === null || val === undefined)
        return null;
    return toIntegerLike(val, msg);
}
function toNumberLike(val, msg) {
...
function toOptionalNumberLike(val, msg) {
    if (val === null || val === undefined)
        return null;
