...
ORDER BY g.course_id, ug.group_id
      `, [teacherId]);
        const arrayOfclassSummaryNeedTeacher = res.rows.map(row => ({
            classId: row.classid,
    ...
    async getDetailNeedReviewOfOneClassForTeacher(teacherId, classId) {
      ...
      `, [classId]);
        ...
        for (const row of res.rows) {
            if (!row.student_id || !row.week_number)
                continue;
            let taskType = null;
            if (!row.score) {
...
exports.PgLearningRepositoryTeacherStats = PgLearningRepositoryTeacherStats;
