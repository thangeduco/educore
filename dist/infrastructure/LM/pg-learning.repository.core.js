...
exports.PgLearningRepositoryCore = void 0;
// src/infrastructure/repositories/pg-learning.repository.core.ts
const db_postgres_1 = require("../common/db-postgres");
class PgLearningRepositoryCore {
    async getStudentBadgeCountForCourse(studentId, courseId) {
        const res = await db_postgres_1.pool.query(`SELECT COUNT(1) AS count
       FROM student_badges
    ...
    async giveBadgeForStudent(studentId, courseId, weekId, videoId, badgeType, adwardedReason) {
        await db_postgres_1.pool.query(`INSERT INTO student_badges (id, student_id, course_id, course_week_id, event_id, badge_type, awarded_reason, awarded_at)
       VALUES (nextval('student_badges_id_seq'::regclass), $1, $2, $3, $4, $5, $6, NOW())`, [studentId, courseId, weekId, videoId, badgeType, adwardedReason]);
    }
    async getSumWeekProgressOfStudentInCourse(studentId, courseId) {
        const res = await db_postgres_1.pool.query(`
      SELECT 
    ...
    async getWeeklyProgressOfStudentInCourse(studentId, courseId) {
        const res = await db_postgres_1.pool.query(`
    ...
    async getWeeklyLearningStatsForStudent(studentId, courseId, weekId) {
        const res = await db_postgres_1.pool.query(`
    SELECT 
    ...
    async getCurrentWeekId(studentId, courseId) {
        const res = await db_postgres_1.pool.query(`SELECT course_week_id
      FROM video_sessions
      where student_id = $1 and course_id = $2
    ...
    async getFirstWeekId(courseId) {
        const res = await db_postgres_1.pool.query(`select id  from course_weeks
        where course_id = $1
    ...
    async stopVideoSession(params) {
        await db_postgres_1.pool.query(`
    ...
    async getLatestAttemptNumber(studentId, worksheetId) {
        const result = await db_postgres_1.pool.query(`SELECT attempt_number FROM worksheet_submissions
     WHERE student_id = $1 AND worksheet_id = $2
     ORDER BY submitted_at DESC NULLS LAST
     LIMIT 1`, [studentId, worksheetId]);
        const row = result.rows[0];
        return row ? Number(row.attempt_number) : null;
    }
    async insertCourseWeekProgress(data) {
        await db_postgres_1.pool.query(`INSERT INTO course_week_progress
    ...
    async submitWorksheet(submission) {
        await db_postgres_1.pool.query(`INSERT INTO worksheet_submissions (
    ...
    async reviewWorksheet(data) {
        await db_postgres_1.pool.query(`UPDATE worksheet_submissions
       SET score = $1,
           review = $2,
           review_at = now (),
           teacher_id = $3,
    ...
    async getWorksheetSubmissionById(id) {
        const res = await db_postgres_1.pool.query(`SELECT id, student_id, worksheet_id, course_week_id, attempt_number, 
    ...
    async getClassViewForTeacher(teacherId, classId) {
        const res = await db_postgres_1.pool.query(`SELECT * FROM class_views WHERE teacher_id = $1 AND class_id = $2`, [teacherId, classId]);
        return res.rows;
    }
    //getClassSummaryForTeacher
    async getClassSummaryForTeacher(teacherId, classId) {
        const res = await db_postgres_1.pool.query(`SELECT * FROM class_summaries WHERE teacher_id = $1 AND class_id = $2`, [teacherId, classId]);
        return res.rows;
    }
    //getNearProgressSummaryForStudent(studentId: number, courseId: number): Promise<StudentCourseWeekForParent[]>
    async getNearProgressSummaryForStudent(studentId, courseId) {
        const res = await db_postgres_1.pool.query(`SELECT * FROM student_course_week_progress
       WHERE student_id = $1 AND course_id = $2
       ORDER BY course_week_id DESC`, [studentId, courseId]);
        return res.rows;
    }
    //createVideoChoiceQuizLog
    async createVideoChoiceQuizLog(params) {
        await db_postgres_1.pool.query(`INSERT INTO video_choice_quiz_logs (
    ...
    async get7DayTeacherReviewsOfStudent(studentId, courseId) {
        const sql = `
    WITH base AS (
      SELECT 
        teacher_reviews,
  ...
  `;
        const res = await db_postgres_1.pool.query(sql, [studentId, courseId]);
        return res.rows.map((row) => ({
    ...
    async get7DayBadgesOfStudent(studentId, courseId) {
        const sql = `
    SELECT 
  ...
  `;
    ...
    async get7DayVideosOfStudent(studentId, courseId) {
        const sql = `
    SELECT 
  ...
  `;
    ...
    async get7DayHomeworkOfStudent(studentId, courseId) {
        const sql = `
    SELECT 
    ...
    ORDER BY get_day
  `;
        const res = await db_postgres_1.pool.query(sql);
        return res.rows.map(row => ({
            get_day: row.get_day, // đã là string YYYY-MM-DD
    ...
    async get7DayVideoChoiceQuizCorrectCountOfStudent(studentId, courseId) {
        const sql = `
    SELECT 
...
exports.PgLearningRepositoryCore = PgLearningRepositoryCore;
