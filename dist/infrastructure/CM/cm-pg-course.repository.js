...
exports.CMPgCourseRepository = void 0;
const db_postgres_1 = require("../common/db-postgres");
const cm_course_model_1 = require("../../domain/CM/models/cm-course.model");
const utils_1 = require("../../shared/utils");
const TABLE_NAME = 'cm_courses';
class CMPgCourseRepository {
    ...
    async getCourseByCode(courseCode) {
      ...
      LIMIT 1
    `;
            const result = await db_postgres_1.pool.query(sql, [courseCode]);
            if (!result.rows || result.rows.length === 0) {
                console.warn('[CMPgCourseRepository][getCourseByCode] ❌ Không tìm thấy khoá học:', courseCode);
                return null;
            }
            // Dùng parser giống style bm-product
            return (0, cm_course_model_1.parseCMCourseRow)(result.rows[0]);
        }
        catch (err) {
            console.error('[CMPgCourseRepository][getCourseByCode] ❌ Lỗi khi query cm_course', err);
            throw err;
        }
    }
    async getNumberOfWeeksOfCourse(courseId) {
        const res = await db_postgres_1.pool.query(`SELECT COUNT(1) AS count FROM course_weeks WHERE course_id = $1`, [courseId]);
        const count = res.rows[0]?.count ?? 0;
        return Number(count);
    }
    async getCourseWeekDetailContents(courseId) {
        const result = await db_postgres_1.pool.query(`
       SELECT 
         wc.id,
         wc.course_week_id,
         cw.week_number,
       ...
       `, [courseId]);
        return result.rows.map((row) => ({
            ...
            step: row.step,
            videoLecture: row.content_type === 'video' && row.video_id
                ? {
                    id: row.video_id,
                ...
                : undefined,
            worksheet: row.content_type === 'worksheet' && row.worksheet_id
                ? {
                    id: row.worksheet_id,
    ...
    async findById(videoId) {
        const res = await db_postgres_1.pool.query(`SELECT * FROM video_lectures WHERE id = $1`, [videoId]);
        return res.rows[0] ?? null;
    }
    async getCourseWeekInfo(weekId) {
    ...
    async getWorksheetInfo(worksheetId) {
        const res = await db_postgres_1.pool.query(`
    SELECT 
      id, 
      title, 
    ...
    async getVideoQuizEvents(studentId, videoId) {
...
ORDER BY ve.id, ve.start_time`, [studentId, videoId]);
        ...
        res.rows.forEach(row => {
            ...
            if (row.quiz_event_id) {
                const quizStep = {
                    ...
                    content: row.content,
                    choices: (0, utils_1.safeParseArray)(row.choices).map((choice) => ({
                        id: choice.id,
                        text: choice.text,
                ...
                if (Array.isArray(videoEventsMap[row.video_event_id].event_data)) {
                    videoEventsMap[row.video_event_id].event_data.push(quizStep);
                }
            }
        });
        // Chuyển đổi từ map sang mảng
        const videoEvents = Object.values(videoEventsMap);
    ...
    async getVideoAudioEvents(studentId, videoId) {
        const res = await db_postgres_1.pool.query(`SELECT 
    ve.id AS video_event_id,
    ve.video_id,
    ve.event_type,
    ...
    order by ve.start_time`, [studentId, videoId]);
    ...
    async getVideoVideoEvents(studentId, videoId) {
        const res = await db_postgres_1.pool.query(`SELECT 
    ve.id AS video_event_id,
    ve.video_id,
    ve.event_type,
    ...
    async getVideoImageEvents(studentId, videoId) {
        const res = await db_postgres_1.pool.query(`SELECT 
    ve.id AS video_event_id,
    ve.video_id,
    ve.event_type,
...
exports.CMPgCourseRepository = CMPgCourseRepository;
